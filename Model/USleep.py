...
class USleep(nn.Module):
    ...
    ----------
    in_chans : int
        Number of EEG/Ear-EEG channels. Set to 3.
    sfreq : float
        EEG sampling frequency. Set to 200.
    depth : int
        Number of conv blocks in encoding layer (number of 2x2 max pools)
        Note: each block halve the spatial dimensions of the features. Set to 12.
    n_time_filters : int
        Initial number of convolutional filters. Set to 5.
    complexity_factor : float
        Multiplicative factor for number of channels at each layer of the U-Net.
        Set to 2.
    with_skip_connection : bool
        If True, use skip connections in decoder blocks. Set to True.
    n_classes : int
        Number of classes. Set to 5.
    input_size_s : float
        Size of the input, in seconds. Set to 30.
    time_conv_size_s : float
        Size of the temporal convolution kernel, in seconds. Set to 9 / 200.
    ...
    apply_softmax : bool
        ...
        False if using nn.CrossEntropyLoss. set to False
    """
    def __init__(self,
                 in_chans=2,
                 sfreq=128,
